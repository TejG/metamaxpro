...
async function streamAnswer({ reasoning = false, temperature = 0.4 } = {}) {
    ...
    try {
        ...
        const config = { ...(reasoning ? GEMINI_THINKING : GEMINI_NO_THINKING), temperature: temperature };

        let response;
        try {
            response = await ai.models.generateContentStream({ model: chosenModel, contents, config });
        } catch (err) {
            // Some "-latest" models reject thinkingBudget overrides → retry without it.
            if (/400|INVALID_ARGUMENT/i.test(err.message || '')) {
