...
export class MainView extends LitElement {
    ...
    _renderPageNavTabs() {
        return html`
            <div class="home-tabs" style="margin-top:16px;">
                <button
                    class="tab-btn ${this._activeNavView === 'main' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'main';
                ...
                <button
                    class="tab-btn ${this._activeNavView === 'ai-customize' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'ai-customize';
                    this.onNavigate('ai-customize');
                }}
                    title="AI Customization"
                >
                    <span class="tab-icon"
                        >${html`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M13 3v7h6l-8 11v-7H5z" /></svg>`}</span
                    >
                    <span class="tab-label">AI</span>
                </button>
                <button
                    class="tab-btn ${this._activeNavView === 'history' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'history';
                ...
                <button
                    class="tab-btn ${this._activeNavView === 'customize' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'customize';
                    this.onNavigate('customize');
                }}
                    title="Settings"
                >
                    <span class="tab-icon"
                ...
                <button
                    class="tab-btn ${this._activeNavView === 'feedback' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'feedback';
                    this.onNavigate('feedback');
                }}
                    title="Feedback"
                >
                    <span class="tab-icon"
                ...
                <button
                    class="tab-btn ${this._activeNavView === 'help' ? 'active' : ''}"
                    @click=${() => {
                    this._activeNavView = 'help';
    ...
    async _loadFromStorage() {
        try {
            const [prefs, creds] = await Promise.all([metaMaxPro.storage.getPreferences(), metaMaxPro.storage.getCredentials().catch(() => ({}))]);
            ...
            <div class="form-group">
                ...
                <div class="key-input-row">
                    <input
                        ...
                        @input=${e => {
                            this._saveAnthropicKey(e.target.value);
                            this._anthropicStatus = 'idle';
                        }}
                    />
                    <button
        ...
        return html`
            ...
            <div class="mode-cards">
                ...
                <div class="mode-card" @click=${() => this._saveMode('anthropic')}>
                    <span class="mode-card-title">Use MetaQuest (Anthropic)</span>
        ...
        return html`
            ...
            <div class="cloud-promo" @click=${() => this._saveMode('cloud')}>
                ...
                <div class="cloud-promo-header">
                    <span class="cloud-promo-title">Switch to MetaQuest Cloud</span>
            ...
            <div class="mode-links">
                ...
                <button class="mode-link" @click=${() => this._saveMode('anthropic')}>Use MetaQuest (Anthropic)</button>
    ...
    _renderAnthropicMode() {
        return html`
            ...
            <div class="form-group">
                ...
                <div class="form-hint">
                    <span class="link" @click=${() => this.onExternalLink('https://console.groq.com/keys')}>Get Groq key</span>
                    &nbsp;— used for Whisper STT ${this._renderKeyStatus(this._groqStatus)}
                </div>
            </div>
            ...
            <div class="form-wrapper">
                ${
                    this._mode === 'local'
                        ? html`
                              <div class="title-row">
                                  <div class="page-title">MetaQuest <span class="mode-suffix">Local AI</span></div>
                                  <button
                                      class="help-btn"
                                      @click=${() => {
                        ...
                        : html`
                            ...
                            this._mode === 'cloud'
                                ...
                                : this._mode === 'anthropic'
                                  ? html`MetaQuest`
                                  : html`MetaQuest <span class="mode-suffix">BYOK</span>`
                        }
                              </div>
                          `
                }
                    ...
                    ${
                        this._mode === 'cloud'
                            ...
                            : this._mode === 'byok'
                              ...
                              : this._mode === 'anthropic'
                                ? 'Use MetaQuest as your interview AI'
                                : 'Run models locally on your machine'
                    }
                </div>

                <div class="top-section">${this._renderTopContent()}</div>
    ...
    _renderTopContent() {
        ...
        switch (this._activeNavView) {
            ...
            default:
                ...
                return html`
                    ${this._renderHomeTabs()} ${this._activeHomeTab === 'start' ? this._renderHomeStartTab() : ''}
